...
function trueman_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = TRUEMAN_STORAGE['co_add_params']!=undefined ? TRUEMAN_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#trueman-customizer-styles-'+option).length > 0 ? jQuery('#trueman-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="trueman-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
